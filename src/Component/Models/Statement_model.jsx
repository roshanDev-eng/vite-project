...
const Statement_model = ({ visible, onClose, data, agents }) => {
  const dispatch = useDispatch();
  const { id } = useParams();
  const [selectedIds, setSelectedIds] = useState([]);
  const { buttonList } = getdispatchfunctios(dispatch);
  const { Transaction_show } = getuseselectervalue();
  const trnasactionfee = useSelector((state) => state.agents.invoiceprofile);
  ...
  const handleGenerate = () => {
    if (!transactionData?.Agents) return;

    // Loop through each selected agent
    selectedAgents.forEach((selectedAgent) => {
      // Find the matching agent from transactionData
      const matchingAgent = transactionData.Agents.find(
        (a) => a.agentId === selectedAgent.Agent_id
      );

      if (matchingAgent) {
        // Extract commission if applicable
        const commission =
          matchingAgent["%"] === "yes" ? matchingAgent.Commission : 0;

        // Dispatch the statement with commission and agent data
        dispatch(
          Statement({
            trnasactionfee: trnasactionfee,
            date: transactionData.Offer_info,
            transaction_id: id,
            agentId: matchingAgent.agentId,
            name: matchingAgent.Name,
            Transaction_price: Number(transactionData.Offer_info.OFFER_PRICE),
            agent_commission: Number(commission),
          })
        );
      }
    });
...
export default Statement_model;
